...
const artist = ref<Artist>(data);
</script>

<template>
    <div class="p-4 overflow-x-hidden" v-if="artist">
        <Button class="text-white text-2xl font-semibold cursor-pointer" :label="artist.name" />

        <div class="py-1.5"></div>
        <div class="flex item-center w-full relative h-full">
            <Image :width="140" :src="artist.albumCover" />
            ...
            <div class="w-full ml-5 flex flex-col gap-2">
                ...
                <div class="flex gap-4 items-center justify-start bottom-0 mb-1.5">
                    <Button v-if="!isPlaying" class="border-0" @click="playFunc()" icon="pi pi-play" aria-label="Play" rounded />
                    <Button v-else class="border-0" @click="playFunc()" icon="pi pi-pause" aria-label="Pause" rounded />
                    <Button class="border-0" icon="pi pi-heart" aria-label="Heart" rounded />
                    <Button class="border-0" icon="pi pi-ellipsis-h" aria-label="DotsHorizontal" rounded />
                </div>
            </div>
        </div>
